...
void Drop::setSpeed(glm::vec3 newSpeed) {
    speed = newSpeed;
}
...
void Drop::updatePosition(glm::vec3 & position, float dtime,
                          Pool & pool, Fountain & fountain) {
    time += dtime * 20.0f;
    if (time > 0.0f) {
		glm::vec3 newPosition;
        newPosition = glm::vec3(speed.x * time,
...
void Fountain::initialize(int levels, int raysPerStep, int dropsPerRay, float dropSize,
	...
	glm::vec3 initialSpeed;  // initial speed for each drop
